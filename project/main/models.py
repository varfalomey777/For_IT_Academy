...
class Post(models.Model):
    car_brand = models.CharField(
        blank=True, null=True, max_length=20, verbose_name="Марка авто "
    )
    car_model = models.CharField(
        blank=True, null=True, max_length=20, verbose_name="Модель авто "
    )
    ...
    photo = models.ImageField(
        verbose_name="Фотография авто ", upload_to="all_photos/%Y/%m/%d/"
    )
    ...
    def __str__(self) -> Any:
        return (
            f"{self.name}, {self.cost}, {self.year}, {self.content}, {self.photo},"
